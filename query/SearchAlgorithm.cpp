...
int BM_Fixed_Align(char* text, int sIdx, int tLen, const char* pattern, BitMap* bitmap, int lineLen, int flag)
{
	...
	{
		return BM_Fixed_AlignR(text, sIdx, tLen, pattern, bitmap, lineLen, flag, en_union_skip);
	}
	else
	{
...
int BM_Fixed_MutiFul(char* text, int sLen, const char* patterns, int patCnt, BitMap* bitmap, int lineLen)
{
...
int BM_Fixed_AlignR(char* text, int sIdx, int tLen, const char* pattern, BitMap* bitmap, int lineLen, int alignType, bool enableUnionSkip)
{
	//if bitmap is already a universal set, return directly
	if(bitmap->BeSizeFul())
	{
    ...
    {  
		...
		{ 
			...
			{  
				if(alignType != 0)
					bitmap->Union(lineNo);
				else if(k-j== lineLen-1 || text[j-1] == ' ')//full match
					bitmap->Union(lineNo);
			}
		}
...
//bm for the same line length, and lineLen = patLen
//type: align type
int BM_Fixed_Pushdown(char* text, int sLen, const char* pattern, BitMap* bitmap, int lineLen, int type)
{
	...
	if(bitmap->BeSizeFul())//if bitmap is a universal set, then use union
	{
		...
		{
			BM_Fixed_Align(text, 0, sLen, pattern, bitmap, lineLen, type);
			return bitmap->GetSize();
		}
		else
		{	
	...
	if(type == QTYPE_ALIGN_RIGHT || type == QTYPE_ALIGN_FULL)
	{
		...
		{
			matchResult = SeqMatching_AlignRight(text + bitmap->GetIndex(i) * lineLen, lineLen, pattern, pLen, type);
			if(matchResult > 0)
			{
...
int BM_Fixed_Pushdown_RefMap(char* text, int sLen, const char* pattern, BitMap* bitmap, BitMap* refBitmap, int lineLen, int type)
{
	...
	else
	{
		...
		if(type == QTYPE_ALIGN_RIGHT || type == QTYPE_ALIGN_FULL)
		{
			for(int i=0;i< bitmapSize;i++)
			{
				matchResult = SeqMatching_AlignRight(text + refBitmap->GetIndex(i) * lineLen, lineLen, pattern, pLen, type);
				if(matchResult > 0)
				{
...
int BM_Fixed_Pushdown_MutiFul(char* text, int sLen, const char* pattern, int patCnt, BitMap* bitmap, int lineLen)
{
	if(bitmap->GetSize() == 0)//if bitmap is empty, return directly
	{
		return 0;
	...
	{
		bitmap->Reset();
		return BM_Fixed_MutiFul(text, sLen, pattern, patCnt, bitmap, lineLen);
	}
...
int BM_Fixed_Pushdown_MutiFul_RefMap(char* text, int sLen, const char* pattern, int patCnt, BitMap* bitmap, BitMap* refBitmap, int lineLen)
{
	if(refBitmap->GetSize() == 0)//if bitmap is empty, return directly
	{
		return 0;
...
int BM_Diff_Pushdown(char* text, int sLen, const char* pattern, BitMap* bitmap, int type)
{
	if(bitmap->GetSize() == 0)//if bitmap is empty, return directly
	{
		return 0;
...
int BM_Diff_Pushdown_RefMap(char* text, int sLen, const char* pattern, BitMap* bitmap, BitMap* refBitmap, int type)
{
	if(refBitmap->GetSize() == 0)//if bitmap is empty, return directly
	{
		return 0;
...
int GetCvars_Diff(char* text, int sLen, OUT char *vars, int entryCnt, int varsLineLen, bool flag)
{
	int index=0; int offset=0;
	int offsetV = 0;
	if(!flag) offsetV += strlen(vars);
	for (char *p= text;*p && p-text< sLen; p++)  
	{
        if (*p == '\n')
		{
			offsetV = index * varsLineLen;
...
int SeqMatching(char * S, int sLen, const char * T, int tLen, int queryType)
{
	int* badc;
	int* goods;
	InitBM(T, badc, goods);
	...
	if(queryType == QTYPE_ALIGN_ANY)
	{
		matchResult = BM_Once(S, T, sLen, badc, goods);//may be 0
	}
	else if(queryType == QTYPE_ALIGN_RIGHT || queryType == QTYPE_ALIGN_FULL)
	{
		matchResult = SeqMatching_AlignRight(S, sLen, T, tLen, queryType);
	}
	else
	{
		matchResult = SeqMatching_AlignLeft(S, sLen, T, tLen);
	}
	return matchResult;  
}
//return  <0: false    1:true
...
int SeqMatching_AlignRight(const char * S, int sLen, const char * T, int tLen, int alignType)
{
	if(sLen < tLen)
	{
		return -2;
	...
	if(alignType != 0)
		return 1;
	else if(sLen == tLen || S[sLen-tLen-1]==' ')
		return 1;
	else
		return -1;
}
...
int MatchInSubpatVvarWithTail(int strTag_mark, int maxlen_mark, const char* patConst, const char* source, int souLen, int& souIndex, int& new_souLen)
{
	int patLen = strlen(patConst);
	LcsMatch lcs= GetLCS_DPoptc(source + souIndex, souLen-souIndex, patConst, patLen);
	if(lcs.Len != patLen) 
	{	//partial match, add at 20230331
		if(lcs.Pos1 > 0)
			new_souLen = souIndex + lcs.Pos1;//matched some char
		else
			new_souLen = souLen;//do not matched, should be the end of the source
		return -1;
	}
		...
		else if(pat->SubSegAttr[i] > SEG_TYPE_SUBVAR)
		{
			...
			else//for variable
			{
				...
				if(i < ePos && pat->SubSegAttr[i+1] == SEG_TYPE_CONST)
				{
					...
					int new_souLen;
					count = MatchInSubpatVvarWithTail(pat->SubVars[i]->tag, pat->SubVars[i]->len, pat->SubSegment[i+1], source, souLen, souIndex, new_souLen);
					if(count == -2) return -2;
					if(count == -1)
					{
						if(INC_TEST_JUDGELEN && new_souLen - tempPos <= pat->SubVars[i]->len)
						{
							souIndex = tempPos;
							count = MatchInSubpatVar_Forward(pat->SubVars[i]->tag, pat->SubVars[i]->len, source, new_souLen, souIndex);
							if(count < 0)
							{
								return -4;
							}
	...
	for(i=sPos; i>=ePos; i--)
	{
		...
		else if(pat->SubSegAttr[i] > SEG_TYPE_SUBVAR)
		{
			...
			else
			{
				...
				{
					int tempPos = souIndex;
					// printf("%d %d %s %s %d\n",pat->SubVars[i]->tag, pat->SubVars[i]->len, pat->SubSegment[i-1], source, souIndex);
	...
	for(i=sPos; i<= ePos; i++)
	{
		...
		else if(pat->segTag[i] > SEG_TYPE_CONST)
		{
			...
			if(i < ePos && pat->segTag[i+1] == SEG_TYPE_CONST)
			{
				int tempPos = souIndex;
				int new_souLen;
				count = MatchInSubpatVvarWithTail(pat->segTag[i], varLen, pat->segCont[i+1], source, souLen, souIndex, new_souLen);
				if(count == -2) return -2;
				if(count == -1)
				{
					souIndex = tempPos;
					if(INC_TEST_JUDGELEN && new_souLen-souIndex > varLen) return -5;
					count = MatchInSubpatVar_Forward(pat->segTag[i], varLen, source, new_souLen, souIndex);
					if(count < 0)
					{
						return -3;
					}			
...
int SubPatternMatch(SubPattern* pat, const char* source, int matchType, OUT RegMatrix* regResult)
{
	...
	switch(matchType)
	{
		case QTYPE_ALIGN_FULL:
		{
			...
			if(matchRst != MATCH_FULL)// if(matchRst < 0)
			{
				regResult->Suby();
				matchRst = -1;
			}
