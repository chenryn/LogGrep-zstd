...
struct groupby_clause : pegtl::seq< kw_by, sp_space, lparen, sp_space, ident, sp_space, rparen, sp_space, pegtl::sor< count_paren, agg_func > > {};
struct timechart_clause : pegtl::seq< kw_timechart, pegtl::star< pegtl::any > > {};
struct right_clause : pegtl::sor< stats_count_by, stats_agg_by, top_clause, distinct_clause, groupby_clause, timechart_clause > {};
...
static bool parse_spl_pegtl(const std::string& right, SPLCommand& out){ pegtl::memory_input in(right, "spl"); grammar_state st; st.out=&out; out.type=SPL_NONE; bool ok = pegtl::parse< right_clause, action >( in, st ); return ok; }
#endif
