...
#define MAX_FILE_LEN		    	1024000//spark=2643046
...
//#define DIR_PATH_DEFAULT		"/apsarapangu/disk9/LogGrepStatic_2/HadoopFastcgi"     //fixed
#define DIR_PATH_DEFAULT		"/root/LogData/LogGrepOut/Hadoop/"
// #define DIR_PATH_DEFAULT		"/root/LogData/LogGrepOut/Spark/"
//#define DIR_PATH_DEFAULT		"/root/LogData/LogGrep/HadoopL7/"
//#define DIR_PATH_DEFAULT		"/apsarapangu/disk3/hive-24hr-converted_c"       //varied
...
#define INC_TEST_SESSION         0      //whether enble session-level optimization
    ...
    for(int i=0;i<ContSize;i++)
    {
      ...
      if(lPos >= 0 && rPos > lPos)
      {
        ...
        else
        {
          ...
          for(int i=0;i<SegSize; i++)
          {
            if(SubSegAttr[i] == SEG_TYPE_CONST)
              SyslogWarnning("%s\n", SubSegment[i]);
            else
              SyslogWarnning("%d\n", SubSegAttr[i]);
          }
...
class BitMap
{
  ...
  public:
    ...
    void Complement(BitMap* target)
    {
      ...
      for(int i=0; i< TotalSize; i++)
      {
        if(Bitmap[i] == 1 && target->Bitmap[i] == 1)
        {
          Reset(i);
        }
      }
    ...
    int GetIndex(int index)
    {
      if(index == idx) return pos;
      idx = index;
      for(int i=pos+1; i< TotalSize; i++)
      {
        if(Bitmap[i] == 1)
        {
          pos = i;
...
typedef struct Statistics
{
  ...
  double total_decom_capsule_time;
  ...
  int hit_at_mainpat_cnt;
  int hit_at_subpat_cnt;
